...
[식재료]
- 계란, 달걀, 유정란, 란, 동물복지란, 자유방목복지란은 식재료로 분류한다.
- 기본 우유류는 식재료로 분류한다.
- 유기농 우유, 비타민우유, 강화우유, 일반 우유, 흰우유, 컵우유, 3컵우유는 식재료로 분류한다.
- 치즈, 아기치즈는 식사 재료 성격이 강하므로 식재료로 분류한다.
- 버터, 메이플버터, 치즈, 햄, 참치, 김, 구운김은 식재료로 분류한다.
- 후추, 순후추, 통후추, 소금, 설탕, 스테비아, 대체당, 감미료, 식초, 간장, 고추장, 된장, 시즈닝, 향신료는 식재료로 분류한다.
- 소스류, 드레싱, 샐러드드레싱, 육수, 동치미육수는 식재료로 분류한다.
- 땅콩버터, 피넛버터, 스키피, 잼, 샹달프, 스프레드류는 식사용/재료 성격이 강하면 식재료로 분류한다.
- 그릭요거트, 무가당요거트, 플레인요거트는 식재료로 분류한다.
- 식빵, 피타브레드, 또띠아처럼 식사용 베이스 성격이 강한 빵류는 식재료로 분류한다.
- 피클, 케이퍼, 황태채, 건어물은 식재료로 분류한다.
- 생고기, 생닭, 생연어, 수산물 원물, 포크립, 포크 립은 식재료로 분류한다.
- 바나나, 사과, 배, 귤, 딸기 등 과일류와 채소류는 식재료로 분류한다.

[간편식]
- 간편식은 완제품 소비 목적이 상품명에 직접 드러날 때 사용한다.
- 라면, 만두, 핫도그, 순댓국, 국/탕류, 전골류, 초밥, 볶음밥, 덮밥은 간편식으로 분류한다.
- 고추튀김, 튀김류, 치킨너겟, 통살치킨은 간편식으로 분류한다.
- 완제품 반찬류, 김치 완제품, 젓갈류, 장조림, 회, 홍어회, 홍어회 모둠은 간편식으로 분류한다.
- 낙지젓, 오징어젓, 명란젓, 창란젓처럼 바로 먹는 젓갈류는 식재료보다 간편식으로 분류한다.
- 크림스프, 스프류는 간단 조리 기반 완제품 소비 성격이 강하므로 간편식으로 분류한다.
- 상품명이 짧은 한글 토큰으로 부자연스럽게 분리되어 있고 완제품/즉석식/조리식 키워드가 명확하지 않으면 간편식으로 분류하지 않는다.
- 예: "오뚜기 순 후 추"처럼 OCR 분절로 조미료명이 끊어진 경우, 간편식이 아니라 식재료 또는 기타로 분류한다.

[간식]
- 꿀약과, 약과, 과자, 스낵, 초콜릿, 빼빼로, 페레로로쉐, 웨하스, 오레오오즈는 간식으로 분류한다.
- 소금버터빵, 베이글, 워터보일드베이글, 페스츄리처럼 디저트/간식 성격이 강한 빵류는 간식으로 분류한다.
...
- 마시는 요거트, 요구르트음료, 비피더스, 빙그레요구르트는 음료로 분류한다.
- 아카페라처럼 RTD 커피/커피 음료 성격이 명확한 상품은 음료로 분류한다.
- 커피, 원두커피, 드립백은 음료로 분류한다.

[주류]
- 필라이트, 테라, 카스, 하이네켄, 맥주, 동동주, 막걸리, 소주, 와인은 주류로 분류한다.
- 테라(페트), 테라 맥주, 국순당쌀막걸리는 주류로 분류한다.
- 단, 카스테라/제과/디저트 문맥 또는 '샹달프& 카스'처럼 식품 조합으로 보이는 경우에는 주류보다 간식/식재료 문맥을 우선한다.

[생활용품]
- 상품명에 칫솔, toothbrush, 치약이 직접 포함되면 생활용품으로 분류한다.
- 세제, 세탁세제, 섬유유연제, 락스, 고무장갑은 생활용품으로 분류한다.
- 랩, 주방랩, 프레스앤씰, 키친타월, 타월, 수건, 휴지, 티슈, 화장지, 물티슈는 생활용품으로 분류한다.
- 종량제봉투, 쓰레기봉투, 재사용봉투, 쇼핑봉투, 장바구니봉투, 봉투20L은 생활용품으로 분류한다.
- 리스테린, 가글, 구강청결제, 구강용품은 생활용품으로 분류한다.
- 샴푸, 린스, 바디워시, 비누는 생활용품으로 분류한다.
- 글라스락, 그리들, 팬, 냄비, 프라이팬 등 주방 조리도구는 생활용품으로 분류한다.

[기타 허용]
- 상품명에 '동물'이라는 단어가 있어도 무조건 반려동물로 분류하지 않는다.
- 반려동물은 사료, 반려동물 간식, 배변패드, 고양이 모래, 반려동물 장난감처럼 목적이 명확한 경우에만 사용한다.
- 의류, 반소매, 티셔츠, 파자마, 자켓, 신발, 전자제품, 잡화, 공구, 망치, 문구류, 사무용품, 필기구, 수정테이프는 기타로 분류한다.
...
[장바구니 문맥]
- 상품명 일부의 향/맛 표현만으로 음료나 식품으로 단정하지 않는다.
- 동일 영수증의 공동 구매 품목(context)을 함께 고려한다.
